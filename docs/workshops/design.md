...
| `workshop-details-panel.tsx` | Group list (member rows + remove) and a read-only rehearsal-date display — editing happens via the Schedule Rehearsal dialog, not inline here. Its rows (`workshop-member-row.tsx`) disable the Part field whenever Type is set to Viewer — a part is an actor concept (attribute 2c), so it shouldn't be editable, or submitted, for a viewer. `add-member-dialog.tsx` and `create-workshop-dialog.tsx`'s own type/part fields follow the same rule. |
